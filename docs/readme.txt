...
Замечено падение бинариков с upx в segfault на некоторых android, когда они запускаются в контексте обычного
...
tpws в режиме socks можно запускать и под более-менее современными билдами windows 10 и windows server 2019+
с установленным WSL. Совсем не обязательно устанавливать дистрибутив убунту, как вам напишут почти в каждой статье про WSL,
которую вы найдете в сети. tpws - статический бинарик, ему дистрибутив не нужен.

Возьмите binaries/x86_64/tpws.
Первым делом нужно распаковать бинарик через upx -d tpws. upx обычно валится под wsl.
Дальше из одного единственного файла tpws с правами 755 нужно создать tar архив : tar cf tpws.tar tpws
Делать это только под *nix ! Под windows вы потеряете права 755.
Переписать tpws.tar на целевую систему. Выполнить : wsl --import tpws "%USERPROFILE%\tpws" tpws.tar
